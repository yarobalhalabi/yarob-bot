...
Ø§Ø®ØªØ± Ø¨Ø§Ù‚ØªÙƒ:"
    ...
    markup.add(types.InlineKeyboardButton("ðŸ”™ Ø±Ø¬ÙˆØ¹", callback_data="back_to_start"))
...
def get_transaction_number(message):
    ...
    if not message.text.isdigit():
        msg = bot.send_message(user_id, "âš ï¸ Ø§Ù„Ø±Ø¬Ø§Ø¡ Ø¥Ø¯Ø®Ø§Ù„ Ø±Ù‚Ù… Ø§Ù„Ø¹Ù…Ù„ÙŠØ© Ø¨Ø´ÙƒÙ„ Ø±Ù‚Ù…ÙŠ ÙÙ‚Ø·.")
    ...
    msg = bot.send_message(user_id, "ðŸ“ž Ø§Ù„Ø±Ø¬Ø§Ø¡ Ø¥Ø¯Ø®Ø§Ù„ Ø§Ù„Ø±Ù‚Ù… Ø§Ù„Ø°ÙŠ Ù‚Ù…Øª Ø¨Ø§Ù„ØªØ­ÙˆÙŠÙ„ Ø¹Ù„ÙŠÙ‡ (`16954304` Ø£Ùˆ `81827789`):")
...
def get_target_number(message):
    ...
    if message.text not in ["16954304", "81827789"]:
        msg = bot.send_message(user_id, "âš ï¸ Ø§Ù„Ø±Ù‚Ù… ØºÙŠØ± ØµØ­ÙŠØ­ØŒ Ø§Ù„Ø±Ø¬Ø§Ø¡ Ø¥Ø¯Ø®Ø§Ù„ Ø£Ø­Ø¯ Ø§Ù„Ø±Ù‚Ù…ÙŠÙ† ÙÙ‚Ø·: `16954304` Ø£Ùˆ `81827789`.")
        return bot.register_next_step_handler_by_chat_id(user_id, get_target_number)
    user_data[user_id]['target_number'] = message.text
    msg = bot.send_message(user_id, "ðŸŽ® Ø£Ø±Ø³Ù„ Ø§Ù„Ø¢Ù† ID Ø­Ø³Ø§Ø¨Ùƒ Ø¯Ø§Ø®Ù„ Ø§Ù„Ù„Ø¹Ø¨Ø©:")
    bot.register_next_step_handler_by_chat_id(user_id, get_game_id)

def get_game_id(message):
    user_id = message.from_user.id
    if not message.text.isdigit():
        msg = bot.send_message(user_id, "âš ï¸ Ø§Ù„Ø±Ø¬Ø§Ø¡ Ø¥Ø¯Ø®Ø§Ù„ ID Ø§Ù„Ù„Ø¹Ø¨Ø© Ø¨Ø´ÙƒÙ„ Ø±Ù‚Ù…ÙŠ ÙÙ‚Ø·.")
...
def confirm_delivery(call):
    ...
    bot.send_message(user_id, "âœ… ØªÙ… ØªÙ†ÙÙŠØ° Ø¹Ù…Ù„ÙŠØ© Ø§Ù„Ø´Ø­Ù† Ø¨Ù†Ø¬Ø§Ø­! Ø´ÙƒØ±Ø§Ù‹ Ù„ØªØ¹Ø§Ù…Ù„Ùƒ Ù…Ø¹Ù†Ø§ ðŸŒŸ")
