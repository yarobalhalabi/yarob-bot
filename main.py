...
ADMIN_ID = 7188219652  # معرف الأدمن
...
def activate_bot(message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        bot.send_message(message.chat.id, "🚫 هذا الأمر مخصص للمشرف فقط.")
...
def choose_game(call):
    ...
    for amount, price in prices.items():
        markup.add(
            types.InlineKeyboardButton(f"{amount} {price_label} - {price}",
...
def handle_selection(call):
    ...
    payment_text = (f"💰 السعر: {prices[amount]}

"
                    "📱 يرجى التحويل عبر سيرياتيل كاش(تحويل يدوي) إلى أحد الأرقام التالية:
"
                    "1️⃣ 0998660515
"
                    "2️⃣ 0936148155
...
"
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(
        types.InlineKeyboardButton("✅ تمت العملية",
                                   callback_data=f"confirm_{user_id}"))
    markup.add(
        types.InlineKeyboardButton("❌ فشلت العملية",
                                   callback_data=f"fail_{user_id}"))

    bot.send_message(ADMIN_ID, final_message, reply_markup=markup)
    bot.send_message(user_id,
                     "✅ تم استلام معلوماتك بنجاح! سيتم تنفيذ طلبك قريبًا 💚")
...
def confirm_delivery(call):
    ...
    bot.send_message(user_id,
                     "✅ تم تنفيذ عملية الشحن بنجاح! شكراً لتعاملك معنا 🌟")
...
def fail_delivery(call):
    ...
    user_id = int(call.data.split("_")[1])
    send_failure_message(user_id)
    bot.answer_callback_query(call.id, "تم إعلام العميل بفشل العملية.")

def send_failure_message(user_id):
    markup = types.InlineKeyboardMarkup()
    markup.add(
        types.InlineKeyboardButton("🔄 إعادة المحاولة", callback_data='retry'))
